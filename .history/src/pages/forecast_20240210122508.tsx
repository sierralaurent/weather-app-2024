    ...
    <main className={`flex min-h-screen flex-col items-center text-black gap-20 p-10`}>
      ...
      {searched && data && (
        <div className={'flex flex-col items-center gap-10 rounded-lg border-black border-8 p-5 bg-slate-500'}>
          <h2 className={'font-bold text-4xl'}>5 day Forecast for {city}</h2>
          {data.list.slice(0, 5).map((forecast: any, index: number) => (
            <div key={index} className={'rounded-lg p-3 bg-slate-300'}>
              <p className='text-3xl font-bold'>{getDayOfWeek(forecast.dt_txt)}</p>
              <Image
                src={weatherImages[forecast.weather[0].main]}
                alt={forecast.weather[0].main}
                width={40}
                height={40} />
              <p className='text-lg'>Weather: {forecast.weather[0].main} - {forecast.weather[0].description}</p>
              <p className='text-xl'>Temperature: {forecast.main.temp} °C</p>
              <p className='text-xl'>Wind speed: {forecast.wind.speed} meter/sec</p>
            </div>
          ))}
          
        </div>
      )}
      <Link href='/'><button className="btn btn-outline btn-secondary hover:scale-110 transition-transform">Go Back to Current Weather</button></Link>
    </main>
    <Footer />
    </>
  );
