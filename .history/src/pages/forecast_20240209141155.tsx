  ...
  return (
    <main className={`flex min-h-screen flex-col items-center text-black gap-20 p-10`}>
        <div className={'flex flex-col gap-5'}>
            ...
            />
            <button onClick={fetchWeatherForecast} className={'rounded-lg border-black border-8 p-3'}>Search</button>
            </div>
      {searched && data && (
        <div className={'flex flex-col gap-10 rounded-lg border-black border-8 p-5 bg-slate-500'}>
          ...
          <Link href='/forecast'><button className='bg-Sage rounded-lg bg-slate-300 p-3 '>Go Back</button></Link>
        </div>
      )}
    </main>
  );
